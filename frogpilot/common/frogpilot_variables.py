...
frogpilot_default_params: list[tuple[str, str | bytes, int, str]] = [
  ...
  ("SshEnabled", "0", 0, "0"),
  ("StartupMessageBottom", "🐸", 0, "Always keep hands on wheel and eyes on road"),
  ("StartupMessageTop", "Drive Safe", 0, "Be ready to take over at any time"),
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme, started):
    ...
    toggle.steerKp = [[0], [np.clip(params.get_float("SteerKP"), steerKp * 0.5, steerKp * 2.5) if advanced_lateral_tuning and is_torque_car and tuning_level >= level["SteerKP"] else steerKp]]
