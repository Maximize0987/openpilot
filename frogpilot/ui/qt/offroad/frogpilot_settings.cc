...
void FrogPilotSettingsWindow::updateVariables() {
  ...
  if (!carParams.empty()) {
    ...
    steerKp = CP.getLateralTuning().which() == cereal::CarParams::LateralTuning::PID ? CP.getLateralTuning().getPid().getKpV()[0] : 1.0;    //  1.0 used to be 0.6  does this force 0.6 as default?
