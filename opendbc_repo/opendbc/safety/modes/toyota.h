...
static bool toyota_tx_hook(const CANPacket_t *msg) {
  const TorqueSteeringLimits TOYOTA_TORQUE_STEERING_LIMITS = {
    ...
    .min_valid_request_rt_interval = 162000,  // 171ms; a ~10% buffer on cutting every 19 frames
  ...
  const int TOYOTA_LTA_MAX_DRIVER_TORQUE = 150; //const int TOYOTA_LTA_MAX_DRIVER_TORQUE = 150;
